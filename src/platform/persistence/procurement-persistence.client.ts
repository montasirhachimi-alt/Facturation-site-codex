...
import type { GoodsReceipt, ProcurementSupplier, PurchaseOrder, PurchaseOrderId, SupplierBill, SupplierPayment } from "@/modules/procurement";
...
export function deleteDraftProcurementPurchaseOrder(purchaseOrderId: PurchaseOrderId) {
  return fetch("/api/persistence/procurement", {
    method: "POST",
    headers: {
      Accept: "application/json",
    ...
    body: JSON.stringify({ operation: "deleteDraftPurchaseOrder", payload: purchaseOrderId })
  }).then(async (response) => {
    if (!response.ok) {
      const body = await response.json().catch(() => undefined) as { error?: string } | undefined;
      throw new Error(body?.error ?? "La suppression du brouillon a échoué.");
    }
    const body = await response.json() as { snapshot?: ProcurementSnapshot };
    if (body.snapshot) applyProcurementSnapshot(body.snapshot);
    return body;
...
export function applyProcurementSnapshot(snapshot: ProcurementSnapshot) {
  ...
  procurementLocalService.replaceSupplierPayments(snapshot.supplierPayments ?? []);
