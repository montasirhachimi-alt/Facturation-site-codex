...
export async function loadHrSnapshot(scope: PersistenceTenantScope): Promise<HrSnapshot> {
  const [departments, positions, employees, contracts, leaveTypes, leaveRequests] = await Promise.all([
...
async function findExistingHrTenant(resource: HrPersistenceResource, id: string) {
  ...
  return prisma.hrLeaveRequest.findUnique({ where: { id }, select: { tenantCompanyId: true } });
}
