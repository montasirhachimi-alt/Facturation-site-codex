    ...
    <SectionCard className="p-4">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">Réunions liées à ce contact pour préparer les prochains échanges.</p>
        </div>
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4 dark:border-hicotech-dark-border dark:bg-hicotech-dark-page/30">
      ...
      <div className="mt-4 space-y-3">
        {meetings.length > 0 ? meetings.map((meeting) => <MeetingCard key={meeting.id} meeting={meeting} />) : <p className="rounded-2xl border border-dashed border-slate-300 bg-white p-4 text-sm text-slate-500 dark:border-hicotech-dark-border dark:bg-hicotech-dark-card dark:text-slate-300">{emptyText}</p>}
      </div>
    </section>
  );
}
